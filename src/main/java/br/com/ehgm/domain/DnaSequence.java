...
public class DnaSequence {
	...
	public int countHorizontalPattern() throws DnaSequenceException {
		...
		for (int r = 0; r < this.matrix.length; r++) {
			...
			LOG.infov("Analyzing horizontal sequences: {0}", Arrays.deepToString(row));
			...
			for (int j = 0; j < row.length; j++) {
				String currentLetter = row[j];
				LOG.debugv("Row letter: {0}", row[j]);
	...
	public int countVerticalPattern() throws DnaSequenceException {
		...
		for (int c = 0; c < width; c++) {
			...
			LOG.infov("Analyzing vertical sequences: {0}", columnLetters);

			String previousLetter = "";
			Map<String, Integer> counters = buildCounters(DNA_VALID_LETTERS);

			// Loop in each letter of column
	...
	public int countMainDiagonalPattern() throws DnaSequenceException {
		...
		for (int d = 0; d <= diagonals; d++) {
			...
			// Collect letters of current diagonal
			...
			LOG.infov("Analyzing main diagonal sequences: {0}", diagonalLetters);

			String previousLetter = "";
			Map<String, Integer> counters = buildCounters(DNA_VALID_LETTERS);

			// Loop in each letter of diagonal
	...
	public int countAntiDiagonalPattern() throws DnaSequenceException {
		...
		for (int d = 0; d <= diagonals; d++) {
			...
			for (int c = 0, k = 0; c <= d; c++, k++) {
				int r = (width - 1) - d + k;
				if (r < height && c < width && r >= 0) {
					diagonalLetters.add(this.matrix[r][c]);
				}
			}
			LOG.infov("Analyzing anti diagonal sequences: {0}", diagonalLetters);
	...
	/**
	 * Check if DNA sequence is valid before do the analysis
	 * @return
	 * @throws DnaSequenceException
	 */
	private boolean checkValidations() throws DnaSequenceException {
